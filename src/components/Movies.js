    ...
    render() {
        ...
        return (
            <>
            {
                ...
                <div>
                    <h3 className="text-center" id="text-underline"><strong>Trending</strong></h3>
                    <div className="movies-list">
                        {
                            this.state.movies.map((movieObj)=>(
                                <div className="card movies-card" onMouseEnter={()=>this.setState({hover:movieObj.id})} onMouseLeave={()=>this.setState({hover:''})}>
                                    ...
                                    <h5 className="card-title movies-title" id="title-underline">{movieObj.original_title}</h5>
                                    {/* <p class="card-text movies-text">{movieObj.overview}</p> */}
                                    <div className="button-wrapper" style={{display: 'flex', width:'100%', justifyContent:'center'}}>
                                    {
                                        this.state.hover == movieObj.id && 
                                        <a class="btn btn-primary movies-button" style={{
                                            backgroundImage: "radial-gradient(136.74% 134.62% at 136.74% -29.44%,#027C89 0%, #6D4650 100%)"
